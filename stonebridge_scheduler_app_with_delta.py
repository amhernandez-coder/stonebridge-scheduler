# Stonebridge Scheduler – Standalone Streamlit App (with Delta Export)
# Features:
# - Works for ANY month/year from uploaded CSVs
# - Strict Year filter + Month/Year multiselect
# - Provider-first Google Calendar CSV (all-day) with Spanish & Private tags
# - SBH-ID added to Description for precise search/delete
...
# - NEW: Delta Export panel to create one-day CSVs (Delta_Add + Delete_Hints)
...
st.caption("Upload rosters, apply rules, and export Google Calendar + Gaps/Violations. Use Delta Export for one-day changes.")
...
# --- On-screen instructions & workflow guide ---
with st.expander("⭐ How to update Google Calendar without touching existing events", expanded=True):
    st.markdown(
        """
        **Use this ‘delta’ method to avoid reimporting a whole month.**

        **To REMOVE a pairing (or Solo)**
        1. In Google Calendar, search the event title on the specific date and delete it.
        2. *(Optional notice)* Import a one-line CSV titled `CANCELLED – <original title>` on that date so providers see the cancellation.

        **To ADD or REPLACE a pairing**
        1. Upload/update your roster for that date (or October pairings) here.
        2. Click **Build Schedule**.
        3. Export **All Appointments** and filter `Start Date` to just the target day in Excel/Sheets.
        4. Save the filtered rows as `Delta_YYYY-MM-DD.csv` (only that date’s rows).
        5. In Google Calendar: delete the outdated event(s) for that day, then import your `Delta_YYYY-MM-DD.csv` into the same calendar.

        **What each CSV row must contain** (the app already formats this):
        - `Subject` – provider-first title (e.g., `Interviewer + Tester - Private - San Antonio Live` or `Interviewer (SOLO) - Telehealth`)
        - `Start Date` and `End Date` – same day in `MM/DD/YYYY`
        - `All Day Event` – `True`
        - `Description` – includes `SBH-ID: YYYY-MM-DD|Site|Interviewer|Tester` for exact search/delete
        - `Location` – full site name

        **FAQ**
        - *Do I import the whole month again?* → **No.** Import only a small **delta CSV** for the impacted day.
        - *How do I remove an interviewer/LPA/Solo?* → Delete that event from Calendar for that date. If replacing, import a delta CSV with the new row(s).
        - *How do I add coverage?* → Add the roster row → Build → Export → Filter to date → Import delta CSV.
        - *Will rules still apply?* → Yes. Lubbock/Gaston, SA psychometrician cap, Amanda Eberle dual testers, and Bobby Perez (LPA-only SA) are enforced when rebuilding.
        """
    )
...
def build_gcal_provider_first_from_pairs(pairs_df: pd.DataFrame,
    ...
    for _, r in pairs_df.iterrows():
        ...
        # Add SBH-ID for precise search/delete later
        start_iso = pd.to_datetime(r["Date"]).strftime("%Y-%m-%d")
        tester_label = tester if (tester and tester != "(SOLO)") else "SOLO"
        sbh_id = f"{start_iso}|{site}|{interviewer}|{tester_label}"
        description_out = (desc + (" | " if desc else "") + f"SBH-ID: {sbh_id}").strip()
...
if build_btn:
    ...
    # ---- Delta Export (one-day change) ----
    with st.expander("⚡ Delta Export (one-day change)", expanded=False):
        st.markdown("Use this to create a one-day CSV you can import without touching other days.")
        if not gcal_all.empty:
            default_date = pd.to_datetime(gcal_all["Start Date"]).min().date()
        else:
            default_date = datetime.today().date()
        sel_date = st.date_input("Date to export", value=default_date)

        if not gcal_all.empty:
            day_mask = pd.to_datetime(gcal_all["Start Date"]).dt.date == pd.to_datetime(sel_date).date()
            day_df = gcal_all[day_mask].copy()
            locs = sorted(day_df["Location"].dropna().unique().tolist())
            loc_choice = st.selectbox("Filter by Location (optional)", ["All"] + locs)
            if loc_choice != "All":
                day_df = day_df[day_df["Location"] == loc_choice]

            st.dataframe(day_df)
            add_bytes = day_df.to_csv(index=False, encoding="utf-8", quoting=csv.QUOTE_MINIMAL).encode("utf-8")
            st.download_button(
                label=f"⬇️ Download Delta_Add ({sel_date})",
                data=add_bytes,
                file_name=f"Delta_Add_{pd.to_datetime(sel_date).strftime('%Y-%m-%d')}.csv",
                mime="text/csv"
            )

            # Delete hints: subjects for quick search/delete of old events
            hints = day_df[["Start Date","Subject","Location"]].copy()
            hints = hints.rename(columns={"Subject":"Subject Search"})
            hint_bytes = hints.to_csv(index=False, encoding="utf-8", quoting=csv.QUOTE_MINIMAL).encode("utf-8")
            st.download_button(
                label=f"⬇️ Download Delete_Hints ({sel_date})",
                data=hint_bytes,
                file_name=f"Delta_Delete_Hints_{pd.to_datetime(sel_date).strftime('%Y-%m-%d')}.csv",
                mime="text/csv"
            )
            st.caption("Before importing Delta_Add, delete any existing event(s) on this date that match these subjects. New events include an SBH-ID in Description for precise search going forward.")
        else:
            st.info("No appointments available. Build the schedule first.")
